...
​	•	**文件描述符（File Descriptor）：** 共享，但文件偏移量是独立的。
...
- 程: 进程是具有一定独立功能的程序，进程是系统资源分配和调度的最小单位。每个进程都有自己的独立内存空间，不同进程通过进程间通信来通信。由于进程比较重量，占据独立的内存，所以上下文进程间的切换开销（栈、寄存器、虚拟内存、文件句柄等）比较大，但相对比较稳定安全。
...
​	•	**软连接**：存储的是目标文件的路径，类似于 Windows 的快捷方式。
...
​	•	**软连接**：可以跨文件系统（不同分区、不同磁盘）。

​	•	**硬连接**：**不能** 跨文件系统，必须在相同的分区内。
...
​	•	**软连接**：拥有自己的 inode，指向目标文件的路径。

​	•	**硬连接**：共享 **inode**，修改其中一个，另一个也会同步更改。
