...
​	•	**优点：** 可执行文件独立，运行时不需要依赖外部库，部署更简单。

​	•	**缺点：** 如果有多个程序都使用了同样的静态库，那么每个程序的可执行文件都包含了一份库代码，浪费存储空间。
...
​	•	**优点：** 如果多个程序使用同一个动态库，则这些程序可以共享库文件，从而节省磁盘空间和内存。动态库可以在程序运行时被更新而无需重新编译整个程序。

​	•	**缺点：** 依赖外部库文件，若库文件丢失或版本不兼容，会导致程序无法运行。程序的部署需要额外考虑这些动态库的路径和版本问题。
