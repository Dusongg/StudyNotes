...
# 8 async/thread/jthread

```cpp
    std::future res = std::async(std::launch::async, [](int a) {
        return a * a;
    }, 10);
    std::future res2 = std::async(std::launch::deferred, [](int a) {
        return a * a + 3;
    }, 10);

    std::packaged_task<int(int)> pkg([](int a) {return a * a + 1;});
    std::future fut = pkg.get_future();
    std::thread t(std::move(pkg), 10);


    std::promise<int> pro;
    std::future<int> fut2 = pro.get_future();
    std::thread t2([&pro](int a) {
        pro.set_value(a * a + 2);
    }, 10);
...
- **`std::jthread`**：
  - 线程在对象生命周期结束时会自动调用 `join()`。即使你忘记手动调用 `join()`，`std::jthread` 的析构函数会确保线程正确地等待完成，避免资源泄漏。

### 2. **默认构造和传递**

- **`std::jthread`**：
  - 提供了更简便的接口，线程对象的构造会自动启动线程。`std::jthread` 的析构函数会确保线程结束时调用 `join()`，你不需要显式地管理线程结束。

### 3. **中断线程**

- **`std::jthread`**：
  - 提供了中断线程的功能。你可以通过 `std::stop_token` 来请求线程停止。线程可以使用 `std::stop_token` 来检查是否请求了停止操作，并在需要时中断执行。

```cpp
#include <iostream>
#include <thread>

void printNumbers(std::stop_token stoken) {
    for (int i = 0; i < 5; ++i) {
        if (stoken.stop_requested()) {
            std::cout << "Thread stopping early." << std::endl;
            return;
        }
        std::cout << i << std::endl;
...
int main() {
    std::jthread t(printNumbers); // 自动启动线程
    // 线程会在 main 函数结束时自动调用 join()，无需显式调用 join()
    return 0;
}
