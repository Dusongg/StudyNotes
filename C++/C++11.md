...
委托构造函数允许使用同一个类中的一个构造函数调用其他的构造函数，**直接在初始化列表中调用其他构造函数**，简化代码

- 注意避免环形调用
- 将构造函数卸载初始化裂变里而不是函数体内部，否则编译器会提示形参的重复定义：*因为构造函数内初始化会先给形参赋值再用新参构造，而初始化列表则是直接构造*
