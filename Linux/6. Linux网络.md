...
- 六个标记位 -> 区分报文的“类型”：
  ...
  - RST：当己方认为链接不一致或链接异常时，要求重新建立链接；RST称为**复位报文段**（建立链接时、或者正在通信服务端异常时）
    ...
    ![image-20240423101816831](C:\Users\ASUS\AppData\Roaming\Typora\typora-user-images\image-20240423101816831.png)
...
## 2.9 read/recv --- write/send

### 2.9.1 向以断开的连接中读数据(server/client)

**正常关闭连接**：

- 当客户端正常关闭连接时，服务器在调用`read`或`recv`时会返回0。这表示已经到达文件结束（EOF），连接已关闭。
- 可以通过检查`read`或`recv`的返回值是否为0来检测这种情况。

**非正常关闭连接**：

- 当客户端非正常关闭连接时，例如客户端崩溃或网络问题，服务器在调用`read`或`recv`时会返回-1，并设置`errno`为`ECONNRESET`，表示连接重置。
- 可以通过检查`read`或`recv`的返回值是否为-1，并检查`errno`是否为`ECONNRESET`来检测这种情况。
...
void handle_server_read(int sockfd) {
    char buffer[1024];
    ssize_t bytes_read;
...
int main() {
    int sockfd = ...;  // 已连接的套接字描述符
    handle_server_read(sockfd);
    close(sockfd);
    return 0;
}
...
### 2.9.2 向以断开的连接中写数据(server/client)

在Linux服务器上，如果向已关闭的客户端连接进行`write`操作，会发生以下情况：

1. **SIGPIPE信号**：默认情况下，当服务器向已关闭的客户端连接写数据时，操作系统会向该进程发送`SIGPIPE`信号。这个信号的默认行为是终止进程，因此如果不捕获和处理`SIGPIPE`信号，服务器进程会被终止。

2. **EPIPE错误**：如果服务器捕获并忽略了`SIGPIPE`信号，或者设置了`SO_NOSIGPIPE`套接字选项以阻止发送`SIGPIPE`信号，`write`调用将返回-1，并将`errno`设置为`EPIPE`，表示管道破裂。

为了处理这种情况，可以采取以下几种措施：

#### 捕获和忽略 SIGPIPE 信号
在程序中捕获并忽略`SIGPIPE`信号，以避免服务器进程被意外终止：
...
#### 使用 SO_NOSIGPIPE 套接字选项（仅适用于BSD系统）
在创建套接字时，设置`SO_NOSIGPIPE`选项来阻止发送`SIGPIPE`信号：
...
#### 使用 MSG_NOSIGNAL 标志
在`send`函数中使用`MSG_NOSIGNAL`标志来阻止发送`SIGPIPE`信号：
...
int main() {
    ...
    // 向已关闭的套接字写入数据
    const char *message = "Hello, client!";
    ssize_t bytes_written = send(sockfd, message, strlen(message), MSG_NOSIGNAL);
...
通过上述方法，可以避免服务器进程因为向已关闭的客户端连接写数据而被意外终止，并且能够正确处理这种错误情况。
